<%@ Page Language="C#"  MasterPageFile="~/pages/MasterPage.master" AutoEventWireup="true" CodeFile="parrains.aspx.cs" Inherits="pages_parrains" %>
                    ...
                    <div id="francemap" >
                        ...
                        <map name="Map" id="theworldmapimage">
                            ...
                            <area shape="poly" id="750" coords="132,51,130,55,128,59,128,64,131,68,132,75,134,80,139,77,144,78,148,81,152,80,154,75,159,74,161,70,160,64,159,60,155,57,152,56,141,53,137,53"
                                ...
                                onclick="zsregister()"  />
                            <area shape="poly" id="D1" coords="40,263,39,261,36,259,35,257,34,254,31,251,27,250,24,250,21,251,19,253,17,255,15,258,17,261,18,265,21,268,25,270,29,271,33,271,37,270,38,267" href="#" />
                            <area shape="poly" id="D2" coords="87,256,83,253,79,249,75,246,70,244,66,244,64,247,61,251,62,255,63,259,66,262,65,266,65,269,64,271,63,274,65,276,68,274,73,274,76,275,78,271,80,267,83,263,86,260" href="#" />
                            <area shape="poly" id="D3" coords="125,260,126,258,124,257,120,254,117,252,113,250,110,252,111,257,113,260,115,263,119,264,120,268,118,269,118,272,124,272,128,274,130,272,128,267,125,264" href="#" />
                            <area shape="poly" id="D4" coords="160,258,157,257,155,256,152,255,153,261,153,264,153,267,155,271,158,271,161,268,161,263,164,261,168,260,172,259,175,259,171,256,169,253,167,250,164,248,164,253,163,256" href="#" />
                        </map>
                    </div>
        ...
        <script type="text/javascript">
            ...
            function dep_clique(nbr) 
                ...
                <table cellspacing="10"> 
                        <td class="RechercheAgent" align="right" style="width: 807px">
                            <strong>
                            Parrains&nbsp;
                            </strong>
                        </td>
                    <tr>
                        <td align="right" class="table_contact" style="width: 807px">
                            Nom de l'agent
                    ...
                    <tr>
                    <td align="right" class="table_contact" style="width: 807px">
                        <asp:Button ID="Button1" runat="server" OnClick="Button1_Click_Tab" Text="Rechercher" />
                        </td>
                </tr>
                    <td align="right" class="table_contact" style="width: 807px">
                        &nbsp;</td>
    ...
    <%
        ...
        {
            sqlDroitAdmin = " AND  (((Clients.contractuel)=True)) ";
            //sqlDroitAdmin = " AND contractuel = true ";
            // (((Clients.contractuel)=True))
        }
        ...
        if (TextBoxNom.Text != "" )
        {
            ...
            sql = "SELECT Clients.id_client, Clients.num_agence, Clients.civilite, Clients.nom_client, Clients.prenom_client, Clients.adresse_client, Clients.postal_client, Clients.postal_client, Clients.ville_client, Clients.tel_client, Clients.tel_client, Clients.fax_client, Clients.société_client, Clients.pass_client, Clients.pays_client, Clients.date_inscription, Clients.statut, Clients.idclient, Clients.idparrain, Clients.contractuel, Clients.num_contrat, Clients.société_client FROM Clients INNER JOIN Clients AS Clients_1 ON Clients.idparrain = Clients_1.idclient WHERE (((Clients.prenom_client) Like '%" + nom + "%')) OR (((Clients.nom_client) Like '%" + nom + "%')) ORDER BY Clients.nom_client DESC;";
            //sql += ") " + sqlDroitAdmin + " ORDER BY Clients.nom_client";
          // SELECT Clients.id_client, Clients.num_agence, Clients.civilite, Clients.nom_client, Clients.prenom_client, Clients.adresse_client, Clients.postal_client, Clients.postal_client, Clients.ville_client, Clients.tel_client, Clients.tel_client, Clients.fax_client, Clients.société_client, Clients.pass_client, Clients.pays_client, Clients.date_inscription, Clients.statut, Clients.idclient, Clients.idparrain, Clients.contractuel, Clients.num_contrat, Clients.société_client
            //FROM Clients INNER JOIN Clients AS Clients_1 ON Clients.idparrain = Clients_1.idclient
            //WHERE (((Clients.prenom_client) Like '* *')) OR (((Clients.nom_client) Like '* ad*'))
            //ORDER BY Clients.nom_client DESC;


            //sql = "SELECT * FROM Clients WHERE ((Clients.[statut]='nego' OR Clients.[statut]='ultranego') AND (Clients.[nom_client] LIKE '%" + nom + "%' OR Clients.[prenom_client] LIKE '%" + nom + "%')";
            //sql += ") " + sqlDroitAdmin + " ORDER BY Clients.nom_client";
        }
        else
        {
            sql = "SELECT Clients.*, IIf([clients].[idparrain]-[clients].[idclient]=0,0,1) AS niveau1 FROM Clients INNER JOIN Clients AS Clients_1 ON Clients.idparrain = Clients_1.idclient ORDER BY Clients.nom_client;";
            //sql = "SELECT * FROM Clients WHERE (Clients.[statut]='nego' OR Clients.[statut]='ultranego' ";
            //sql += ") " + sqlDroitAdmin + " ORDER BY Clients.nom_client";
        }
        //sql +=") "+ sqlDroitAdmin+" ORDER BY Clients.nom_client";
...
<%
    ...
    Response.Write("Page " + Convert.ToString(page) + " sur " + Convert.ToString(nbPage) + " ");
    if (nbPage >= 1)
    {
        Response.Write("<a href=\"./parrains.aspx?page=" + Convert.ToString(page - 1) + "\" >Page précédente <<</a> ");
        if (page == 1)
        {
            Response.Write("<strong><a href=\"./parrains.aspx?page=1\" >1</a></strong>");
        }
        else
        {
            Response.Write("<a href=\"./parrains.aspx?page=1\" >1</a>");
        }
        for (int i = 2; i <= (nbPage); i++)
        {
            if (i == page)
            {
                Response.Write(" | <strong><a href=\"./parrains.aspx?page=" + Convert.ToString(i) + "\" >" + Convert.ToString(i) + "</a></strong>");
            }
            else
            {
                Response.Write(" | <a href=\"./parrains.aspx?page=" + Convert.ToString(i) + "\" >" + Convert.ToString(i) + "</a>");
            }
        }
        Response.Write(" <a href=\"./parrains.aspx?page=" + Convert.ToString(page + 1) + "\" >>> Page suivante</a>");
    }
            %> 
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									Affichage par page<asp:DropDownList ID="DropDownListNbClient" runat="server" CssClass=" tbsanswidth" OnSelectedIndexChanged="DropDownListNbClient_SelectedIndexChanged" AutoPostBack="true" ViewStateMode="Enabled">
			...
			<%
                ...
                while (cl.MoveNext())
                {
                    ...
                    {
                        if (cl.Current.CIVILITE == "Mr")
                        {

                            //sourceJpg = "../img_site/logo_320.jpg";
                            sourceJpg = "../img_site/img_homme.jpg";
                        }
                        else
                        {
                            sourceJpg = "../img_site/img_femme.jpg";
                        }
                    }
                    Response.Write(
                                "<div class=\"Resultat-header2\" >"
                                    ...
                                    + "<div class=\"Resultat-photo\">"
                                        + "<a class=\"lienImage\" href=\"./affichagerecherche.aspx?ref=" + cl.Current.PRENOM_CLIENT + " " + cl.Current.NOM_CLIENT + "\">  <img alt=\"photo\" src= \"" + sourceJpg
                                         + "\"height=\"96\" style=\"border:none; float:left;  height:96px\" /></a>"
                                    + "</div>"

                                    + "<div class=\"Resultat-lien-droit2\">"
                                    //+ ((member != null && member.STATUT == "ultranego" && cl.Current.Contractuel == true)?"Contractuel":"")
                                     + ((member != null && member.STATUT == "ultranego" && cl.Current.Contractuel == true) ? "" : "")
                                     //   + "<a href='./afficherCompte.aspx' ><img src=\"../img_site/editer.png\" alt=\"Google Maps\" /> Associes Noveuax </a>"
                                      //  + "<img src=\"../img_site/editer.png\" alt=\"Google Maps\" /> <input type=\"button\" style=\"width:150px;\" onclick=\"clickLoc('" + cl.Current.PRENOM_CLIENT + " " + cl.Current.NOM_CLIENT + "')\" class=\"myButton\" value=\"Modifier\" /><br />
                                            + "<img src=\"../img_site/icon_user.gif\" alt=\"Google Maps\" /><input type=\"button\" style=\"width:150x;\" onclick=\"clickAch('" + cl.Current.IDCLIENT + "')\" class=\"myButton\" value=\"         Profil         \" /><br />"
                                            + "<img src=\"../img_site/editer.png\" alt=\"Google Maps\" /> <input type=\"button\" style=\"width:150px;\" onclick=\"clickLoc('" + cl.Current.IDCLIENT + "')\" class=\"myButton\" value=\"Associes Noveuax\" /><br />"
                                        //+ "<input type=\"button\" style=\"width:200px;\" onclick=\"clickAch('" + cl.Current.PRENOM_CLIENT + " " + cl.Current.NOM_CLIENT + "')\" class=\"myButton\" value=\"Consulter ses ventes\" /><br />"
                                        //+ "<a href='javascript:carte_google(\"" + cl.Current.VILLE_CLIENT + "\")' ><img src=\"../img_site/loupe_googlemap.png\" alt=\"Google Maps\" /> </a>"
                                        //+ "<a href='javascript:carte_google(\"" + cl.Current.VILLE_CLIENT + "\")' class=\"linkGoogleMaps\" > Localisation<br /></a>"
                                        );
                    ...
                    Response.Write("<Strong>" + cl.Current.NOM_CLIENT.ToUpper() + " " + cl.Current.PRENOM_CLIENT + "</Strong><br />" + "Téléphone: " + cl.Current.TEL_CLIENT + "<br/>");

                    Response.Write("Ville: " + cl.Current.VILLE_CLIENT + "<br/>Code postal: " + cl.Current.POSTAL_CLIENT);
                    string mail = cl.Current.ID_CLIENT;
                    Response.Write("<br />" + "Mail: <A HREF=mailto:" + cl.Current.ID_CLIENT + ">" + cl.Current.ID_CLIENT);
...
%>
<%--<%
    Response.Write("<tr id=\"pageBas\"><td>");
    Response.Write("Page " + Convert.ToString(page) + " sur " + Convert.ToString(nbPage) + " ");
    if (nbPage >= 1)
    {
        Response.Write("<a href=\"./Recherche_agent.aspx?page=" + Convert.ToString(page - 1) + "\" >Page précédente <<</a> ");
        if (page == 1)
        {
            Response.Write("<strong><a href=\"./Recherche_agent.aspx?page=1\" >1</a></strong>");
        }
        else
        {
            Response.Write("<a href=\"./Recherche_agent.aspx?page=1\" >1</a>");
        }
        for (int i = 2; i < (nbPage) + 1; i++)
        {
            if (i == page)
            {
                Response.Write(" | <strong><a href=\"./Recherche_agent.aspx?page=" + Convert.ToString(i) + "\" >" + Convert.ToString(i) + "</a></strong>");
            }
            else
            {
                Response.Write(" | <a href=\"./Recherche_agent.aspx?page=" + Convert.ToString(i) + "\" >" + Convert.ToString(i) + "</a>");
            }
        }
        Response.Write(" <a href=\"./Recherche_agent.aspx?page=" + Convert.ToString(page + 1) + "\" >>> Page suivante</a>");
    }
    ...
    %>--%>
                    ...
                    <asp:Textbox runat="server" ID="Txtmail" CssClass="invisible" hidden="true" />
                    <asp:Button runat="server" ID="btnLoc" CssClass="invisible" onClick="clickLoc" hidden="true" />
                    <asp:Button runat="server" ID="btnAch" CssClass="invisible" onClick="clickAch" hidden="true" />
        <script type="text/javascript">
        ...
        function clickLoc(nomClient) {
            var btn = document.getElementById('<%=btnLoc.ClientID %>');
        ...
        function clickAch(nomClient) {
            var btn = document.getElementById('<%=btnAch.ClientID %>');
            var txt = document.getElementById('<%=txt.ClientID %>');
            txt.value = nomClient;
            btn.click();
        }
